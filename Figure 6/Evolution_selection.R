...
genes_dNdS=filter(genes_dNdS,ds > 0.0005 & dn > 0.0005 & w < 999)
...
for(i in 1:length(type)){
  cur = exp_list[[i]][exp_list[[i]]$WBGene %in% unique(positive$WBGene),]
  exp_enr[i,"gene_positive"] = length(unique(cur$WBGene))
  exp_enr[i,"total"] = length(unique(exp_list[[i]]$WBGene))
  exp_enr[i,"percentage"] = exp_enr[i,"gene_positive"]/exp_enr[i,"total"]
  exp_enr[i,"enrich"] = -log10(phyper(c(exp_enr[i,"gene_positive"]-1),exp_enr[i,"total"],c(19997 - exp_enr[i,"total"]),length(unique(positive$WBGene)),lower.tail = F))
}
