...
###dS distribution
pdf("ds.pdf")
plot(ecdf(genes_dNdS$ds),verticals = TRUE,do.points =F,ylab="Cumulative Fraction",xlab="dS",xlim=c(0,3),main="",col="#212121",lwd=6,cex.lab=1.5,cex.axis=2)
lines(ecdf(genes_dNdS[genes_dNdS$wb %in% unique(cls1$WBGene),]$ds),verticals = TRUE, do.points =F,col="#D53E4F",lwd=6)
lines(ecdf(genes_dNdS[genes_dNdS$wb %in% unique(cls3$WBGene),]$ds),verticals = TRUE, do.points =F,col="#3288BD",lwd=6)
lines(ecdf(genes_dNdS[genes_dNdS$wb %in% unique(cls2$WBGene),]$ds),verticals = TRUE, do.points =F,col="#FFA138",lwd=6)
lines(ecdf(genes_dNdS[genes_dNdS$wb %in% unique(Cel_dup_N4Cel$WBGene),]$ds),verticals = TRUE, do.points =F,col="#5E4FA2",lwd=6)
lines(ecdf(genes_dNdS[genes_dNdS$wb %in% unique(Cel_dup_N1N3$WBGene),]$ds),verticals = TRUE, do.points =F,col="grey",lwd=6)
legend("bottomright",legend=c("all_genes","cls1","cls2","cls3","N4Cel","N3N1"),lty=c(1,1),col=c("#212121","#D53E4F","#3288BD","#FFA138","#5E4FA2","grey"),ncol=1,bty="n",inset=0.03,cex=1.6,lwd=6,text.font = 2)
dev.off()

###ks test
Cel_dup_N4Cel$type = "n4cel"
Cel_dup_N1N3$type = "N3N1"
g1 = list(genes_dNdS$ds,genes_dNdS[genes_dNdS$wb %in% rownames(cls1),]$ds,
          genes_dNdS[genes_dNdS$wb %in% rownames(cls2),]$ds,genes_dNdS[genes_dNdS$wb %in% rownames(cls3),]$ds,
          genes_dNdS[genes_dNdS$wb %in% unique(Cel_dup_N4Cel$WBGene),]$ds,
          genes_dNdS[genes_dNdS$wb %in% unique(Cel_dup_N1N3$WBGene),]$ds)
type = c("all","cls1","cls2","cls3","n4cel","N3N1")
alt = combn(1:6,2)
...
write.csv(g1_p,file = "ds_cls1_2_3_n4cel_ks_test.csv",row.names = F)

###expression of Cluster1,Cluster2 and Cluster3 genes
